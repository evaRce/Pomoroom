defmodule Pomoroom.UsersTest do
  ...
  test "registers a user" do
    ...
    assert user1.email == "email_1@h.es"
    assert user1.nickname == "nickname_1"
    assert user1.image_profile != nil
    assert String.match?(user1.image_profile, ~r/^\/images\/default_user\/default_user-.*\.svg$/)
  end
